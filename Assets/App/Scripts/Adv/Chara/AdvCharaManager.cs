...
namespace Adv.Chara
{
	...
	public class AdvCharaManager
	{
		...
		private Dictionary<string, AdvCharaData> _dictChara = new Dictionary<string, AdvCharaData>();   // キャラ名をKeyとしてデータを保持する
		...
		public AdvCharaData GetData(string name)
		{
			AdvCharaData data = null;
		...
		/// </summary>
		/// <returns></returns>
		private AdvCharaData DataCheck(string name)
		{
			AdvCharaData data = null;

			if (!_dictChara.TryGetValue(name, out data))
			{
				data = new AdvCharaData(name);
